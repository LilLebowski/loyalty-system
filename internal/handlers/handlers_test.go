...
func TestRegisterHandler(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			request := httptest.NewRequest(http.MethodPost, "/api/user/auth/register", bytes.NewBuffer(marshalledData))
...
func TestLoginHandler(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			request := httptest.NewRequest(http.MethodPost, "/api/user/auth/login", bytes.NewBuffer(marshalledData))
			w := httptest.NewRecorder()
			ctrl := gomock.NewController(t)
			strg := mock_storage.NewMockStorage(ctrl)
			strg.
